...
must_haves:
  truths:
    ...
    - "EDC column set includes subject, sitenumber, site, sitegroup (4 additions)"
    - "All existing tests still pass"
  artifacts:
    - path: "src/astraea/transforms/usubjid.py"
      provides: "NaN-safe USUBJID generation"
    - path: "src/astraea/profiling/profiler.py"
      provides: "Updated EDC column set with 4 new columns (29 total)"
...
<objective>
Fix USUBJID NaN-to-"nan" bug and update EDC system column set (add 4 missing columns).
...
<task type="auto">
  ...
  **Fix 2: EDC column set in profiler.py**
  ...
  The current EDC_SYSTEM_COLUMNS frozenset has 25 entries. The following are ALREADY present and must NOT be re-added or "fixed":
  - "studyenvsitenumber" (correct spelling, no typo exists)
  - "foldername" (already in the set)
  - "studysiteid" (already in the set)

  Add ONLY these 4 truly missing columns (all lowercase since matching is case-insensitive):
  - "subject"
  - "sitenumber"
  - "site"
  - "sitegroup"

  The final frozenset should have 29 entries (25 existing + 4 new).
  </action>
  <verify>
  ```
  python3 -c "
  ...
  assert len(EDC_SYSTEM_COLUMNS) == 29, f'Expected 29, got {len(EDC_SYSTEM_COLUMNS)}'
  assert 'subject' in EDC_SYSTEM_COLUMNS, 'subject missing'
  assert 'sitenumber' in EDC_SYSTEM_COLUMNS, 'sitenumber missing'
  assert 'site' in EDC_SYSTEM_COLUMNS, 'site missing'
  assert 'sitegroup' in EDC_SYSTEM_COLUMNS, 'sitegroup missing'
  # Verify pre-existing columns still present
  assert 'studysiteid' in EDC_SYSTEM_COLUMNS, 'studysiteid missing'
  assert 'foldername' in EDC_SYSTEM_COLUMNS, 'foldername missing'
  assert 'studyenvsitenumber' in EDC_SYSTEM_COLUMNS, 'studyenvsitenumber missing'
  print('OK: EDC columns updated (29 total)')
  print('ALL CHECKS PASSED')
  "
  ```
  </verify>
  <done>USUBJID rejects NaN/empty components. EDC column set has 29 entries with 4 new columns added.</done>
</task>
...
<task type="auto">
  ...
  **Create new file tests/test_profiling/test_profiler_unit.py** (unit tests not dependent on Fakedata):
  ...
  Import `_is_edc_column` using its full path: `from astraea.profiling.profiler import _is_edc_column, EDC_SYSTEM_COLUMNS`

  Add:
  1. `test_edc_columns_count`: assert len(EDC_SYSTEM_COLUMNS) == 29
  ...
  9. `test_edc_correct_spelling`: "studyenvsitenumber" in EDC_SYSTEM_COLUMNS
  10. `test_clinical_column_not_edc`: _is_edc_column("AETERM") returns False
...
3. EDC column detection confirmed for all 4 new columns
</verification>
...
- EDC_SYSTEM_COLUMNS has 29 entries: 25 existing + 4 new (subject, sitenumber, site, sitegroup)
