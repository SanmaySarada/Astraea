...
must_haves:
  truths:
    - "All 19 wrong codelist assignments in domains.json now reference correct C-codes from Plan 01"
...
<objective>
Fix all domains.json errors: 19 wrong codelist assignments, 15 wrong core designations, DA domain class, 16+ null codelist assignments, and add natural key_variables for 9 core domains.
...
<task type="auto">
  ...
  **1. Fix 19 wrong codelist assignments:**
  ...
  **5. Fill 16+ null codelist assignments:**
  ...
  - VSPOS: null -> "C71148" (if variable exists; if not, skip -- note: C71148 is Position, but this may not be in the CT file yet. Assign the code anyway for correctness.)
  ...
  **6. Add key_variables to each domain:**
  ...
  "LB": { "key_variables": ["STUDYID", "USUBJID", "LBTESTCD", "VISITNUM", "LBDTC"], ... }
  ...
  **IMPORTANT:** Check the DomainSpec Pydantic model in src/astraea/models/sdtm.py to see if it already has a key_variables field. If not, add `key_variables: list[str] | None = None` to the model FIRST, then update domains.json. If the model already supports it, just update the JSON.
  ...
  assert d['LB'].get('key_variables') is not None, 'LB keys missing'

  # Check AE codelist fixes
  ae_vars = {v['name']: v for v in d['AE']['variables']}
  assert ae_vars['AESEV']['codelist_code'] == 'C66769', f'AESEV: {ae_vars[\"AESEV\"][\"codelist_code\"]}'
  assert ae_vars['AESER']['codelist_code'] == 'C66742', f'AESER: {ae_vars[\"AESER\"][\"codelist_code\"]}'
  ...
  <done>All codelist assignments, core designations, DA class, null codelists, ARMNRS, and key_variables corrected.</done>
</task>
...
<task type="auto">
  ...
  **Structural tests:**
  ...
  - `test_lb_key_variables`: LB.key_variables includes "LBTESTCD"
...
2. Verify cross-reference: every codelist_code in domains.json has a matching key in codelists.json (except codes for codelists not yet added like C65047, C67154, etc. -- these are forward references that will be populated in a future phase)
</verification>

<success_criteria>
- All 19 codelist assignment errors fixed in domains.json
- All 15 core designation errors fixed
- DA domain class is Findings
- 16+ null codelist assignments filled
