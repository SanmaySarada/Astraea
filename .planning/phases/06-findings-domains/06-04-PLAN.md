...
must_haves:
  truths:
    ...
    - "All Findings domains (LB, EG, PE, VS) produce valid .xpt files with correct variable labels and sort order"
    - "Cross-domain USUBJID validation covers all new Findings domains"
    - "Date imputation flags (--DTF/--TMF) survive XPT roundtrip for LB and EG domains"
  artifacts:
    ...
    - path: "tests/integration/execution/test_findings_xpt_output.py"
      provides: "XPT output tests for LB, EG, PE, VS domains + SUPPQUAL with date imputation flag roundtrip"
  key_links:
...
<objective>
Integration tests for SUPPQUAL generation and XPT output for all Findings domains -- the final verification layer ensuring all Phase 6 execution components work end-to-end, including VS domain XPT output and date imputation flag roundtrip.

Purpose: SUPPQUAL referential integrity is a common source of P21 validation failures. XPT output tests verify that generated datasets survive write-read roundtrip. This plan ties together the TRANSPOSE handler, FindingsExecutor, SUPPQUAL generator, and XPT writer. VS domain XPT output is tested even though no raw data exists -- proving the pipeline can handle VS when data is available.

Output: Integration tests verifying SUPPLB/SUPPEG generation and XPT output for LB, EG, PE, VS.
</objective>
...
<task type="auto">
  <name>Task 2: Findings domain XPT output tests including VS and date imputation flags</name>
  <files>
    tests/integration/execution/test_findings_xpt_output.py
  </files>
  <action>
...
1. Create fixtures for LB, EG, PE, VS SDTM DataFrames (minimal but valid):
   - LB: 6 rows with STUDYID, DOMAIN, USUBJID, LBSEQ, LBTESTCD, LBTEST, LBORRES, LBORRESU, LBSTRESC, LBSTRESN, LBSTRESU, LBNRIND, LBDTC, LBDTF (date imputation flag: "D" for 2 rows with partial dates, empty for complete dates)
   - EG: 4 rows with STUDYID, DOMAIN, USUBJID, EGSEQ, EGTESTCD, EGTEST, EGORRES, EGORRESU, EGSTRESC, EGSTRESN, EGDTC, EGTPT, EGDTF (date imputation flag: "D" for 1 row)
   - PE: 3 rows with STUDYID, DOMAIN, USUBJID, PESEQ, PETESTCD, PETEST, PEORRES, PEDTC
   - VS: 4 rows with STUDYID, DOMAIN, USUBJID, VSSEQ, VSTESTCD, VSTEST, VSORRES, VSORRESU, VSSTRESC, VSSTRESN, VSSTRESU, VSPOS, VSDTC
...
2. Tests (use tmp_path for temporary XPT files):
   ...
   - **test_vs_xpt_roundtrip: Write VS DataFrame to XPT, read back with pyreadstat, verify all columns (including VSPOS) and row count match**
   ...
   - test_cross_domain_findings_usubjid: Create DM + LB + EG + PE + VS DataFrames, run DatasetExecutor.validate_cross_domain_usubjid, verify all USUBJIDs consistent
   - **test_lb_date_imputation_flag_roundtrip: Verify LBDTF column with "D" values survives XPT write-read. Assert that 2 rows have LBDTF="D" and remaining rows have LBDTF="" after roundtrip.**
   - **test_eg_date_imputation_flag_roundtrip: Verify EGDTF column survives XPT roundtrip with "D" value for 1 row.**
  </action>
  <verify>
...
Run `pytest tests/ -x -q` -- full suite passes.
  ...
  <done>
XPT roundtrip tests pass for LB, EG, PE, VS, and SUPPLB. Variable labels survive write-read. Cross-domain USUBJID validation covers all Findings domains including VS. Date imputation flags (LBDTF, EGDTF) survive XPT roundtrip. Full test suite remains green.
  </done>
</task>
...
- XPT roundtrip successful for all Findings domains including VS
- Date imputation flags survive XPT roundtrip for LB and EG
</verification>
...
3. All Findings domain XPT files (LB, EG, PE, VS) survive write-read roundtrip
4. Cross-domain USUBJID validation covers LB, EG, PE, VS
5. Date imputation flags (--DTF) present and survive XPT roundtrip
