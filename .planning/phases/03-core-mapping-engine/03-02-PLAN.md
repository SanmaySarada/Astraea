...
must_haves:
  truths:
    ...
    - "Context output for DM domain is under 20KB (focused context, not bloated)"
  artifacts:
...
# Plan 01 creates the mapping models this plan imports
@.planning/phases/03-core-mapping-engine/03-01-SUMMARY.md
</context>
...
Import StudyMetadata from astraea.models.mapping (created in Plan 01, which runs first in this wave since Plan 02 depends on Plan 01).
  </action>
  <verify>
...
2. **Tests:**
   ...
   - `test_build_prompt_output_size`: Assert `len(output.encode('utf-8')) < 20480` for a DM-sized context (verifies focused context, not bloated)
...
Use real SDTMReference and CTReference from bundled data for realistic tests.
  ...
  <done>Context builder is tested with realistic DM domain data, correctly filters EDC columns, and output stays under 20KB.</done>
</task>
...
<success_criteria>
MappingContextBuilder produces well-structured LLM prompts with only relevant context (clinical vars, domain-specific CT, matched eCRF forms), and EDC system columns are excluded. Output stays under 20KB for DM domain.
</success_criteria>
