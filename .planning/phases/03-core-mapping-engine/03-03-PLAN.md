...
must_haves:
  truths:
    ...
    - "TRANSPOSE pattern is defined in system prompt for forward-compatibility but not exercised until Phase 6 (Findings domains require it)"
  artifacts:
...
<task type="auto">
  ...
  <files>src/astraea/mapping/prompts.py</files>
  <action>
...
1. **MAPPING_SYSTEM_PROMPT** -- a string constant containing the system prompt for the mapping agent. It should:
   ...
   - Note: Include TRANSPOSE in the pattern list for forward-compatibility. Describe it as "Wide -> tall transformation for Findings domains (LB, VS, EG). Not applicable to Events/Interventions/Special Purpose domains."
...
2. **MAPPING_USER_INSTRUCTIONS** -- a string template appended after the context builder output, containing:
  ...
  <verify>
    `python -c "from astraea.mapping.prompts import MAPPING_SYSTEM_PROMPT, MAPPING_USER_INSTRUCTIONS; assert 'TRANSPOSE' in MAPPING_SYSTEM_PROMPT; print('Prompts importable')"` succeeds.
    `ruff check src/astraea/mapping/prompts.py` -- no lint errors.
  </verify>
  <done>System prompt defines mapping specialist role with all 9 pattern descriptions including forward-compatible TRANSPOSE.</done>
</task>

<task type="auto">
  <name>Task 2: Create validation/enrichment module with tests</name>
  <files>src/astraea/mapping/validation.py, tests/unit/mapping/test_validation.py</files>
  <action>
**validation.py:**
...
- Use real bundled reference data for realistic tests (SDTMReference, CTReference)
  ...
  <verify>
    ...
    `ruff check src/astraea/mapping/validation.py` -- no lint errors.
  </verify>
  <done>Validation module catches CT errors, enriches proposals with SDTM-IG metadata, and adjusts confidence scores. Tests verify all adjustment rules.</done>
</task>

<task type="auto">
  <name>Task 3: Create MappingEngine orchestrator</name>
...
<success_criteria>
MappingEngine.map_domain() produces a validated, enriched DomainMappingSpec from an LLM proposal. CT validation catches invalid terms. Confidence adjustments work correctly. All without making real API calls in tests. TRANSPOSE pattern is documented in system prompt for Phase 6 forward-compatibility.
</success_criteria>
