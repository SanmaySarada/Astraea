...
must_haves:
  truths:
    ...
    - "All new rules default to ERROR severity per FDA v1.6 guidance"
  artifacts:
...
Purpose: Only 6 of ~45 FDAB rules are implemented. Adding 14+ more covers the most common FDA validation findings. All new rules use ERROR severity per FDA v1.6 guidance (no severity = all findings matter).
...
- All new rules use ERROR severity (except date ordering and paired warnings)
