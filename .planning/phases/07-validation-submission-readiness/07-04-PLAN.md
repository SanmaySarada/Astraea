...
must_haves:
  truths:
    ...
    - "Mapping engine integrates predict-and-prevent results into mapping output"
  artifacts:
...
Output: validation/predict.py with spec-level checks, wired into mapping/engine.py
</objective>
...
This function consolidates the existing mapping/validation.py logic into formal RuleResult objects. The existing validate_and_enrich() and check_required_coverage() functions remain unchanged (they serve the mapping pipeline). predict_and_prevent provides a second, more formal layer.

**Tests**: Create synthetic DomainMappingSpec objects with known issues (duplicate variables, bad CT codes, missing required vars, ASSIGN with wrong CT values). Verify each check independently. Verify returned RuleResult objects have correct rule_ids, severities, and messages.
  </action>
  <verify>pytest tests/unit/validation/test_predict.py -x -q</verify>
  <done>Predict-and-prevent catches 7 types of spec-level issues before dataset generation</done>
</task>
...
1. Add import: `from astraea.validation.predict import predict_and_prevent`
...
3. Store the predict-and-prevent results on the spec or return them alongside the spec. The cleanest approach: add an optional `predict_prevent_issues: list[str]` field to DomainMappingSpec (or return as separate value). Check existing return signature and adapt.
4. Log any ERROR-level issues from predict_and_prevent using loguru.
5. Do NOT block mapping on predict-and-prevent failures -- these are informational for human review. Log warnings, include in output.
...
Verify: Run existing mapping tests to confirm no regression: `pytest tests/unit/mapping/ -x -q`
  ...
  <done>Mapping engine calls predict-and-prevent after building spec; existing mapping tests still pass</done>
</task>
...
- `ruff check src/astraea/validation/predict.py src/astraea/mapping/engine.py`
