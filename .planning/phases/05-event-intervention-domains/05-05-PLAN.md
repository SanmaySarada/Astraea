...
must_haves:
  ...
  artifacts:
    - path: "tests/integration/execution/test_cross_domain_validation.py"
      provides: "Cross-domain USUBJID validation and --DY integration test"
      min_lines: 80
...
<objective>
Validate cross-domain consistency and XPT output generation across all Phase 5 domains. This plan proves: (1) cross-domain USUBJID validation catches orphan subjects, (2) --DY calculation works with cross-domain RFSTDTC lookup, and (3) all domains can produce valid .xpt files via execute_to_xpt.

Purpose: Individual domain tests in Plans 02-04 prove each domain in isolation. This plan proves they work TOGETHER -- consistent USUBJIDs across domains, study day derivation from DM reference dates, and actual XPT file output.

Output: Two integration test files proving cross-domain coherence and XPT output.
</objective>
...
<task type="auto">
  <name>Task 1: Cross-domain USUBJID validation and --DY integration test</name>
...
ae_spec and raw_ae_df fixtures: Minimal AE spec with just STUDYID, DOMAIN, USUBJID, AESEQ, AETERM, AESTDTC (enough for --DY testing). Same 3 subjects as DM. Include date columns for --DY calculation.
...
- test_dy_without_cross_domain_context: Execute without CrossDomainContext -- AESTDY should not be present or should be NaN (graceful handling)
  ...
  <done>Cross-domain USUBJID validation catches orphans. --DY calculation produces correct study day values from cross-domain RFSTDTC. 6+ tests passing.</done>
</task>
