...
class VariableMappingProposal(BaseModel):
    ...
    mapping_pattern: MappingPattern = Field(
        ..., description="How the source maps to the target"
    )
...
class DomainMappingProposal(BaseModel):
    ...
    domain: str = Field(
        ..., description="Target SDTM domain code (e.g., 'AE', 'DM')"
    )
    ...
    mapping_notes: str = Field(
        default="", description="General notes about the mapping approach"
    )
...
class VariableMapping(BaseModel):
    ...
    sdtm_label: str = Field(
        ..., description="SDTM variable label from the IG spec"
    )
    sdtm_data_type: Literal["Char", "Num"] = Field(
    ...
    source_variable: str | None = Field(
        default=None, description="Source variable name"
    )
    ...
    codelist_code: str | None = Field(
        default=None, description="NCI codelist code if applicable"
    )
    codelist_name: str | None = Field(
        default=None, description="NCI codelist name if applicable"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score (0.0 to 1.0)"
    )
...
class DomainMappingSpec(BaseModel):
    ...
    domain_label: str = Field(
        ..., description="Domain description (e.g., 'Adverse Events')"
    )
    domain_class: str = Field(
        ..., description="Domain class (e.g., 'Events', 'Findings')"
    )
    structure: str = Field(
    ...
    total_variables: int = Field(
        ..., ge=0, description="Total number of mapped variables"
    )
    required_mapped: int = Field(
        ..., ge=0, description="Number of Req variables with mappings"
    )
    expected_mapped: int = Field(
        ..., ge=0, description="Number of Exp variables with mappings"
    )
    high_confidence_count: int = Field(
        ..., ge=0, description="Number of HIGH confidence mappings"
    )
    medium_confidence_count: int = Field(
        ..., ge=0, description="Number of MEDIUM confidence mappings"
    )
    low_confidence_count: int = Field(
        ..., ge=0, description="Number of LOW confidence mappings"
    )
    mapping_timestamp: str = Field(
        ..., description="ISO 8601 timestamp of when mapping was performed"
    )
    model_used: str = Field(
        ..., description="LLM model identifier used for mapping"
    )
    unmapped_source_variables: list[str] = Field(
        default_factory=list,
        description="Source variables not mapped to any SDTM target",
