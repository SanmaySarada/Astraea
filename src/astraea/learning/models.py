...
class MappingExample(BaseModel):
    ...
    mapping_logic: str = Field(..., description="Human-readable mapping logic description")
    ...
    final_mapping_json: str = Field(..., description="Serialized VariableMapping JSON")
...
class CorrectionRecord(BaseModel):
    ...
    original_pattern: str = Field(..., description="Original mapping pattern proposed")
    corrected_pattern: str | None = Field(
        default=None, description="Corrected mapping pattern (None for reject)"
    )
    original_logic: str = Field(..., description="Original mapping logic proposed")
...
class StudyMetrics(BaseModel):
    ...
    study_id: str = Field(..., description="Study identifier")
    domain: str = Field(..., description="SDTM domain code")
    total_proposed: int = Field(..., ge=0, description="Total number of proposed mappings")
    approved_unchanged: int = Field(..., ge=0, description="Mappings approved without changes")
    corrected: int = Field(..., ge=0, description="Mappings that were corrected")
    rejected: int = Field(..., ge=0, description="Mappings that were rejected")
    ...
    completed_at: str = Field(..., description="ISO 8601 timestamp of review completion")
